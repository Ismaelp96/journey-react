...
export function SelectInvities({ onClose }: SelectInvitiesProps) {
  const [getNumberToInvite, setGetNumberToInvite] = useState(['31995820947']);
  const [getEmailToInvite, setGetEmailToInvite] = useState([
    'ismael6991@live.com',
  ]);

  function removeEmailFromInvites(emailToRemove: string, telToRemove: string) {
    const newEmailList = getEmailToInvite.filter(
      (email) => email !== emailToRemove
    );

    const newNumberList = getNumberToInvite.filter(
      (number) => number !== telToRemove
    );
              ...
              >
                <div className='flex items-center gap-2'>
                  ...
                  <button
                    type='button'
                    onClick={() =>
                      removeEmailFromInvites(email, getNumberToInvite[i])
                    }
                  >
